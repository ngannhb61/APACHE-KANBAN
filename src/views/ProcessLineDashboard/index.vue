...
<script lang="ts" setup>
  ...
  productionRate.value = reactive({
    ...
    data: [],
    index: false,
    columnWidth: [105, 95, 95, 100, 110, 95, 95, 95, 95],
    align: [
      'center',
      'center',
      'center',
  ...
  abnormalQuality.value = reactive({
    header: [
      '月份',
      '<div><div>内部责任</div><div>批量异常</div></div>',
      '<div><div>内部责任</div><div>翻箱异常</div></div>',
      '<div><div>内部责任</div><div>重大异常</div></div>',
      '<div style="color:#32C5E9"><div>外部责任</div><div>批量异常</div></div>',
      '<div style="color:#32C5E9"><div>外部责任</div><div>翻箱异常</div></div>',
      '<div style="color:#32C5E9"><div>外部责任</div><div>重大异常</div></div>',
    ],
    data: [],
    index: false,
  ...
  const getGroupMemberInfo = async () => {
    ...
    if (result.data) {
      ...
      if (skillWorkers !== 0 && jockeyWorks !== 0 && waterSpider !== 0) {
        const data: any = [];
        data.push({ name: '技能工', value: result.data.skill_workers });
        data.push({ name: '操作工', value: result.data.jockey_works });
        data.push({ name: '水蜘蛛', value: result.data.water_spider });
